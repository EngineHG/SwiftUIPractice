...
class Store: ObservableObject {
  ...
  static func reduce(state: AppState, action: AppAction) -> (AppState, AppCommand?) {
    ...
    case .showSafariView(let url):
      appState.pokemonList.isSFViewActive = (url, true)
    case .closeSafariView:
      appState.pokemonList.isSFViewActive = (nil, false)
      
    case .selectTabIndex(index: let index):
      appState.mainTab.selection = index
      
    case .setFavorite(let pokemon):
      appCommand = FavoriteCommand(pokemon: pokemon, isFavorite: true)
      
    case .showLoginTip:
      appState.pokemonList.needShowLoginTip = true
